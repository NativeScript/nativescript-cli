...
export class CacheStore {
  ...
  constructor(collectionName, options = { tag: undefined, useDeltaSet: false, useAutoPagination: false, autoPagination: false, autoSync: true }) {
  ...
  find(query, options: any = { autoSync: false }) {
  ...
  group(aggregation, options = { autoSync: false }) {
    const autoSync = options.autoSync === true || this.autoSync;
    const cache = new DataStoreCache(this.collectionName, this.tag);
    const stream = Observable.create(async (observer) => {
      try {
  ...
  async create(doc, options = { autoSync: false }) {
    if (isArray(doc)) {
  ...
  async update(doc, options = { autoSync: false }) {
    if (isArray(doc)) {
  ...
  async remove(query, options = { autoSync: false }) {
    const autoSync = options.autoSync === true || this.autoSync;
    const cache = new DataStoreCache(this.collectionName, this.tag);
    const sync = new Sync(this.collectionName, this.tag);
    let count = 0;
  ...
  async removeById(id, options = { autoSync: false }) {
    const autoSync = options.autoSync === true || this.autoSync;
    const cache = new DataStoreCache(this.collectionName, this.tag);
    const sync = new Sync(this.collectionName, this.tag);
    let count = 0;
  ...
  async pull(query, options = { useDeltaSet: false, useAutoPagination: false, autoPagination: false }) {
    const network = new NetworkStore(this.collectionName);
    const cache = new DataStoreCache(this.collectionName, this.tag);
    const queryCache = new QueryCache(this.tag);
    ...
    if (useAutoPagination) {
      ...
      // Create the pages
      const pageSize = (options as any).autoPaginationPageSize || (options.autoPagination && (options.autoPagination as any).pageSize) || PAGE_LIMIT;
      ...
      const pageCounts = await Promise.all<number>(pagePromises);
  ...
  async sync(query?: Query, options?) {
