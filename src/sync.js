    ...
    if (syncEntities.length > 0) {
      ...
      const batchSync = async (syncResults) => {
        const promise = new Promise(async resolve => {
          ...
          const results = await Promise.all(map(batch, async syncEntity => {
            const collection = syncEntity.collection;
            let entity = syncEntity.entity;
            ...
            if (method === HttpMethod.DELETE) {
              ...
              try {
                ...
                if (metadata.isLocal()) {
                  ...
                  putNetworkRequest.method = HttpMethod.POST;
                  putNetworkRequest.url = url.format({
                    protocol: this.client.protocol,
                    host: this.client.host,
          ...
          if (i < syncEntities.length) {
            return resolve(await batchSync(syncResults));
          }
