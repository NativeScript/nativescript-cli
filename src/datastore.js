...
class SyncStore extends DataStore {
  /**
   * Find all entities in the data store. A query can be optionally provided to return
   * a subset of all entities in a collection or omitted to return all entities in
   * a collection. The number of entities returned adheres to the limits specified
...
class CacheStore extends DataStore {
  constructor(collection, options) {
    super(collection, options);
    this.networkStore = new NetworkStore(collection, options);
    this.syncStore = new SyncStore(collection, options);
  }
  ...
  find(query, options = {}) {
    const stream = KinveyObservable.create(async observer => {
      try {
        ...
        let syncCount = await this.syncStore.syncCount(null, options);
        if (syncCount > 0) {
          await this.syncStore.push(null, options);
          syncCount = await this.syncStore.syncCount(null, options);
        }
        ...
        const cacheEntities = await this.syncStore.find(query, options).toPromise();
        ...
        await super.remove(removeQuery, options);

        // Save network data to cache
        await super.save(networkEntities, options);
  ...
  findById(id, options) {
    const stream = KinveyObservable.create(async observer => {
      try {
        ...
        const cacheEntity = await this.syncStore.findById(id, options).toPromise();
        ...
        await super.save(networkEntity, options);

        // Emit the network entity
        observer.next(networkEntity);
      } catch (error) {
        if (error instanceof NotFoundError) {
          await super.removeById(id, options);
        }
