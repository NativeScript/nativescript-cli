...
export class DebugIOSCommand implements ICommand {
	...
	constructor(protected $errors: IErrors,
		...
		// Do not dispose ios-device-src, so the process will remain alive and the debug application (NativeScript Inspector or Chrome DevTools) will be able to connect to the socket.
		// In case we dispose ios-device-src, the socket will be closed and the code will fail when the debug application tries to read/send data to device socket.
