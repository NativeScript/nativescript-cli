    ...
    popup.onLoaded((event) => {
      try {
        if (event.url && event.url.indexOf(redirectUri) === 0 && redirected === false) {
          ...
          const { code = undefined, error = undefined, error_description = undefined } = parsedUrl.query || {};
          ...
          if (code) {
            ...
            reject(new KinveyError(error as string, error_description));
          } else {
...
async function getTokenWithCode(code, clientId, redirectUri, options: any = {}) {
  const { authProtocol, authHost } = getConfig();
...
async function getTokenWithUsernamePassword(username, password, clientId, options: any = {}) {
  const { authProtocol, authHost } = getConfig();
  const request = new KinveyRequest({
    method: RequestMethod.POST,
    headers: {
...
export async function loginWithRedirectUri(redirectUri, options: any = {}) {
...
export async function loginWithUsernamePassword(username, password, options: any = {}) {
  const { appKey } = getConfig();
  const { micId } = options;
  let clientId = appKey;
