...
export function createRequest(method: string, url: string, body?: any): KinveyRequest {
...
export class NetworkStore {
  ...
  async removeById(id, options: any = {}) {
    const { apiProtocol, apiHost } = getConfig();
    const {
      rawResponse = false,
      timeout,
  ...
  async subscribe(receiver, options: any = {}) {
    const { apiProtocol, apiHost } = getConfig();
    const {
      timeout,
      properties,
