...
class QueryCache extends Cache {
  ...
  async findByKey(key) {
    const query = new _kinveyQuery.Query().equalTo('query', key);
  ...
  async save(query, response) {
    ...
    if (key !== null) {
      const headers = new _kinveyHttp.KinveyHeaders(response.headers);
...
class Sync {
    ...
    if (docs.length > 0) {
      ...
      if (event === SyncEvent.Delete) {
        docs = docs.filter(doc => {
          const kmd = new _kinveyKmd.Kmd(doc._kmd);
      ...
      const query = new _kinveyQuery.Query().contains('_id', docs.map(doc => doc._id));
  ...
  async push(query) {
    const network = new _networkstore.NetworkStore(this.appKey, this.collectionName);
    ...
    if (syncDocs.length > 0) {
      ...
      const batchPush = async pushResults => {
        ...
        const results = await Promise.all(batch.map(async syncDoc => {
          const _id = syncDoc._id,
                _syncDoc$state = syncDoc.state,
                state = _syncDoc$state === void 0 ? {
            operation: undefined
          } : _syncDoc$state;
          ...
          if (event === SyncEvent.Delete) {
            try {
              ...
              await network.removeById(_id); // Remove the sync doc

              await syncCache.removeById(_id); // Return a result
            ...
            try {
              let local = false; // Get the doc from the cache

              let doc = await cache.findById(_id); // Save the doc to the backend
              ...
              await syncCache.removeById(_id); // Save the doc to cache
  ...
  async pull(query) {
    const network = new _networkstore.NetworkStore(this.appKey, this.collectionName);
    const cache = new Cache(this.appKey, this.collectionName, this.tag);
    const queryCache = new QueryCache(this.appKey, this.collectionName, this.tag); // Find the docs on the backend

    const response = await network.find(query, true).toPromise();
    const docs = response.data; // Update the cache

    await cache.save(docs); // Update the query cache

    queryCache.save(query, response); // Return the number of docs
      ...
      try {
        ...
        if (queryCacheDoc && queryCacheDoc.lastRequest) {
          ...
          if (Array.isArray(deleted) && deleted.length > 0) {
            const removeQuery = new _kinveyQuery.Query().contains('_id', deleted.map(doc => doc._id));
          ...
          await queryCache.save(query, response); // Return the number of changed docs
  ...
  async autopaginate(query, options = {}) {
    const network = new _networkstore.NetworkStore(this.appKey, this.collectionName);
    const cache = new Cache(this.appKey, this.collectionName, this.tag);
    const queryCache = new QueryCache(this.appKey, this.collectionName, this.tag); // Clear the cache

    await cache.clear(); // Get the total count of docs

    const response = await network.count(query, true).toPromise();
    const count = 'count' in response.data ? response.data.count : Infinity; // Create the pages

    const pageSize = options.autoPaginationPageSize || options.autoPagination && options.autoPagination.pageSize || PAGE_LIMIT;
    const pageCount = Math.ceil(count / pageSize);
    const pageQueries = (0, _times.default)(pageCount, i => {
      const pageQuery = new _kinveyQuery.Query(query);
    ...
    const pagePromises = pageQueries.map(pageQuery => {
      return network.find(pageQuery).toPromise().then(docs => cache.save(docs)).then(docs => docs.length);
    });
    const pageCounts = await Promise.all(pagePromises);
    const totalPageCount = pageCounts.reduce((totalCount, pageCount) => totalCount + pageCount, 0); // Update the query cache

    queryCache.save(query, response); // Return the total page count
