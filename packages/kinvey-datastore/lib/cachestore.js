...
class CacheStore {
  constructor(appKey, collectionName, tag, options = {
    useDeltaSet: false,
  ...
  find(query, options = {}) {
    ...
    const cache = new _cache.DataStoreCache(this.appKey, this.collectionName, this.tag);
  ...
  findById(id, options = {}) {
    ...
    const stream = _kinveyObservable.KinveyObservable.create(async observer => {
      try {
        ...
        if (autoSync) {
          const query = new _kinveyQuery.Query().equalTo('_id', id);
          await this.pull(query, options);
  ...
  async create(doc, options = {}) {
    const autoSync = options.autoSync === true || this.autoSync;
    const cache = new _cache.DataStoreCache(this.appKey, this.collectionName, this.tag);
    const sync = new _sync.Sync(this.appKey, this.collectionName, this.tag);
    const cachedDoc = await cache.save(doc);
    await sync.addCreateSyncEvent(cachedDoc);

    if (autoSync) {
      const query = new _kinveyQuery.Query().equalTo('_id', cachedDoc._id);
  ...
  async remove(query, options = {}) {
    ...
    if (docs.length > 0) {
      ...
      if (syncDocs.length > 0 && autoSync) {
        const pushQuery = new _kinveyQuery.Query().contains('_id', syncDocs.map(doc => doc._id));
  ...
  async removeById(id, options = {}) {
    ...
    if (doc) {
      ...
      if (autoSync) {
        const query = new _kinveyQuery.Query().equalTo('_id', doc._id);
        const pushResults = await this.push(query);
  ...
  async pull(query, options = {}) {
    ...
    const autoSync = options.autoSync === true || this.autoSync;
    const sync = new _sync.Sync(this.appKey, this.collectionName, this.tag); // Push sync queue
    ...
    if (count > 0) {
      ...
      // }
      if (count === 1) {
        throw new Error(`Unable to pull entities from the backend. There is ${count} entity` + ' that needs to be pushed to the backend.');
      }

      throw new Error(`Unable to pull entities from the backend. There are ${count} entities` + ' that need to be pushed to the backend.');
    } // Delta Set


    if (useDeltaSet) {
      return sync.deltaset(query, Object.assign({
        useDeltaSet,
        useAutoPagination,
        autoSync
      }, options));
    ...
    if (useAutoPagination) {
      return sync.autopaginate(query, Object.assign({
        useDeltaSet,
        useAutoPagination,
        autoSync
      }, options));
