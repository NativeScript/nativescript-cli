...
export class Doctor implements NativeScriptDoctor.IDoctor {
	...
	public async canExecuteLocalBuild(
		...
		if (
			platform.toLowerCase() === Constants.ANDROID_PLATFORM_NAME.toLowerCase()
		) {
	...
	public async getWarnings(
		...
		return info
			.filter((item) => item.type === Constants.WARNING_TYPE_NAME)
	...
	private getAndroidInfos(
		...
		result = result.concat(
			...
			this.processSysInfoItem({
				...
				warningMessage:
					"The Java Development Kit (JDK) is not installed or is not configured properly.",
				additionalInformation:
	...
	private async getiOSInfos(
		...
		if (this.hostInfo.isDarwin) {
			result = result.concat(
				...
				this.processSysInfoItem({
					item:
						!sysInfoData.cocoaPodsVer || !sysInfoData.isCocoaPodsUpdateRequired,
	...
	private processSysInfoItem(data: {
		item: string | boolean;
		infoMessage: string;
	...
	private processValidationErrors(data: {
		warnings: NativeScriptDoctor.IWarning[];
		infoMessage: string;
		platforms: string[];
	}): NativeScriptDoctor.IInfo[] {
