...
function getXcodeProjectArgs(data?: { hasProjectWorkspace: boolean, hasTarget?: boolean }) {
	return data && data.hasProjectWorkspace ? [
		"-workspace", path.join(projectRoot, `${projectName}.xcworkspace`),
		"-scheme", projectName
	] : [
		"-project", path.join(projectRoot, `${projectName}.xcodeproj`),
		data && data.hasTarget ? "-target" : "-scheme", projectName
	];
}
			...
			_.each(["INFO", "TRACE"], logLevel => {
				_.each(["Debug", "Release"], configuration => {
					it(`should return correct args when workspace is ${hasProjectWorkspace} with ${logLevel} log level and ${configuration} configuration`, async () => {
						const injector = createTestInjector({ logLevel, hasProjectWorkspace });

						const buildConfig = { buildForDevice: false, release: configuration === "Release" };
						const xcodebuildArgsService = injector.resolve("xcodebuildArgsService");
						const actualArgs = await xcodebuildArgsService.getBuildForSimulatorArgs({ projectRoot }, { projectName }, buildConfig);
		...
		const testCases = [
			{
				...
				connectedDevices: [{deviceInfo: {activeArchitecture: "arm64"}}, {deviceInfo: {activeArchitecture: "armv7"}}],
				expectedArgs: ["ONLY_ACTIVE_ARCH=NO", "-sdk", "iphoneos"].concat(getCommonArgs())
			},
			{
				...
				_.each(["INFO", "TRACE"], logLevel => {
					_.each(["Debug", "Release"], configuration => {
						it(`${testCase.name} when hasProjectWorkspace is ${hasProjectWorkspace} with ${logLevel} log level and ${configuration} configuration`, async () => {
							const injector = createTestInjector({ logLevel, hasProjectWorkspace, connectedDevices: testCase.connectedDevices });

							const platformData = { projectRoot, getBuildOutputPath: () => buildOutputPath };
							const projectData = { projectName };
							const buildConfig = { buildForDevice: true, release: configuration === "Release" };
							const xcodebuildArgsService: IXcodebuildArgsService = injector.resolve("xcodebuildArgsService");
							const actualArgs = await xcodebuildArgsService.getBuildForDeviceArgs(<any>platformData, <any>projectData, <any>buildConfig);
