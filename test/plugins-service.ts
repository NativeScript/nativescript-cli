...
function createAndroidManifestFile(projectFolder: string, fs:IFileSystem): void {
    ...
    '<permission android:name="android123"></permission>' + 
    '<permission android:name="android456"></permission>' + 
...
describe("Plugins service", () => {
	...
	describe("merge xmls tests", () => {
		...
		it("merges AndroidManifest.xml and produces correct xml", () => {
			...
			// Asserts that backup file exists
			let backupFilePath = path.join(appDestinationDirectoryPath, "original-AndroidManifest.xml");
			assert.isTrue(fs.exists(backupFilePath).wait());
		});
	});
});