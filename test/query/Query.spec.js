...
describe('Kinvey.Query', function() {
  ...
  before(function(done) {
    ...
    this.complex = new Kinvey.Entity({
      name: 'John',
      surname: 'Smith',
      age: 50,
    ...
    // Save both.
    this.complex.save(callback(done, {
      success: function() {
        simple.save(callback(done));
      }
    }));
  });
  ...
  beforeEach(function() {
    ...
    this.collection = new Kinvey.Collection(COLLECTION_UNDER_TEST, { query: this.query });
  });

  // Remove all created data.
  after(function(done) {
    var complex = this.complex;
    this.simple.destroy(callback(done, {
      success: function() {
        complex.destroy(callback(done, {
          success: function() {
    ...
    it('performs a matching all query.', function(done) {
      ...
      this.collection.fetch(callback(done, {
        success: function(list) {
          ...
          list[0].toJSON(true).should.eql(complex.toJSON(true));
          done();
        }
      }));
    });
  ...
  describe('.exist', function() {
    ...
    it('performs a negative exist query.', function(done) {
      ...
      this.collection.fetch(callback(done, {
        success: function(list) {
          list.should.have.length(1);
          list[0].toJSON(true).should.eql(simple.toJSON(true));
          done();
        }
      }));
    });
    ...
    it('sorts by field (ascending).', function(done) {
      ...
      this.collection.fetch(callback(done, {
        success: function(list) {
          ...
          list[0].toJSON(true).should.eql(simple.toJSON(true));
          list[1].toJSON(true).should.eql(complex.toJSON(true));
          done();
        }
      }));
    });
    it('sorts by field (descending).', function(done) {
      ...
      this.collection.fetch(callback(done, {
        success: function(list) {
          list.should.have.length(2);
          list[0].toJSON(true).should.eql(complex.toJSON(true));
          list[1].toJSON(true).should.eql(simple.toJSON(true));
          done();
        }
      }));
    });
