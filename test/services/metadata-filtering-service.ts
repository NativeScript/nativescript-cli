...
describe("metadataFilteringService", () => {
	...
	describe("generateMetadataFilters", () => {
		const mockFs = (input: {
			...
			readJsonData?: any;
			writeFileAction?: (filePath: string, data: string) => void;
			existingFiles?: any[];
		}): { fs: FileSystemStub; dataWritten: IDictionary<any> } => {
			...
			if (input.writeFileAction) {
				fs.writeFile = (filePath: string, data: string) =>
					input.writeFileAction(filePath, data);
			} else {
				fs.writeFile = (filePath: string, data: string) =>
					(dataWritten[filePath] = data);
			}
			...
			if (input.existingFiles) {
				fs.exists = (filePath: string) =>
					input.existingFiles.indexOf(filePath) !== -1;
			}
		...
		const getExpectedWhitelistContent = (input: {
			applicationWhitelist?: string[];
			pluginWhitelist?: string[];
		}): string => {
			let finalContent = "";
			if (input.pluginWhitelist) {
				finalContent += `// Added from: ${pluginNativeApiUsageFilePath}${EOL}${input.pluginWhitelist.join(
					EOL
				)}${EOL}// Finished part from ${pluginNativeApiUsageFilePath}${EOL}`;
			}
			...
			if (input.applicationWhitelist) {
				...
				finalContent += `// Added from application${EOL}${input.applicationWhitelist.join(
					EOL
				)}${EOL}// Finished part from application${EOL}`;
			}
