...
const User = require('../../src/legacy/user');
const AlreadyLoggedInError = require('../../src/core/errors').AlreadyLoggedInError;
const nock = require('nock');
const usersNamespace = process.env.KINVEY_USERS_NAMESPACE || 'user';

describe('User', function() {
  beforeEach(function() {
    sinon.spy(User, 'logout');
  });
  ...
  describe('signup()', function() {
      ...
      return User.getActive().then(user => {
        if (user) {
          ...
          nock(this.client.apiUrl)
            .delete(`/${usersNamespace}/${this.client.appId}/${user._id}`)
            .query(Common.createNockQuery(null, flags))
            .reply(200, reply, {
              'content-type': 'application/json'
            });

          return User.destroy(user._id, flags);
        }
      });
    });
    ...
    it('should create a new user', function() {
      ...
      reply._acl.creator = reply.id;

      nock(this.client.apiUrl)
        .post(`/${usersNamespace}/${this.client.appId}`)
        .reply(201, reply, {
          'content-type': 'application/json'
        });

      const promise = User.signup().then(response => {
    ...
    it('should create a new user with preseeded data', function() {
      ...
      const promise = User.signup(data).then(response => {
    ...
    it('should fail when there is already an active user', function() {
      ...
      return promise.then(() => {
        return User.signup(null, { error: spy });
      }).then(() => {
        return expect(promise).to.be.rejected;
      }, function(err) {
        expect(err).to.be.instanceof(AlreadyLoggedInError);
    ...
    it('should support both deferreds and callbacks on failure', Common.failure(function(options) {
      const reply = {
        name: 'ServerError',
        description: 'An error occurred on the server. Please retry your request.',
        debug: ''
      };

      nock(this.client.apiUrl)
        .get(`/${usersNamespace}/${this.client.appId}`)
        .reply(500, reply, {
          'content-type': 'application/json'
        });
...
*/
  ...
  describe('login()', function() {
    before(function() {
      ...
      return User.create({}, { state: false }).then(user => {
        this.data = user;
      });
    });
    ...
    after(function() {
      return User.setActive(this.data).then(() => {
        const reply = {
          count: 1
        };
        const flags = { hard: true };
        nock(this.client.apiUrl)
          .delete(`/${usersNamespace}/${this.client.appId}/${this.data._id}`)
          .query(Common.createNockQuery(null, flags))
          .reply(200, reply, {
            'content-type': 'application/json'
          });

        return User.destroy(this.data._id, { hard: true });
      }).then(() => {
        return User.setActive(null);
      });
    });
    ...
    it('should reject the promise on invalid arguments', function() {
      const promise = User.login({
        foo: Common.randomString()
      });
      return expect(promise).to.be.rejectedWith('Username and/or password missing.');
    });
    ...
    it('should login a user', function() {
      ...
      nock('https://baas.kinvey.com')
        .post(`/${usersNamespace}/${this.client.appId}/login`)
        .reply(200, reply, {
          'content-type': 'application/json',
        });
    ...
    it('should fail when there is already an active user', function() {
      ...
      return promise.then(() => {
        return User.login(this.data.username, this.data.password, { error: spy });
      }).then(() => {
        return expect(promise).to.be.rejected;
      }).catch(err => {
        expect(err).to.be.instanceof(AlreadyLoggedInError);
    ...
    it('should support both deferreds and callbacks on success', Common.success(function(options) {
      ...
      return User.login(this.data.username, this.data.password, options);
    }));
    ...
    it('should support both deferreds and callbacks on failure', Common.failure(function(options) {
      ...
      nock(this.client.apiUrl)
        .get(`/${usersNamespace}/${this.client.appId}/login`)
        .reply(401, reply, {
          'content-type': 'application/json'
        });

      return User.login(Common.randomString(), Common.randomString(), options);
    }));
  });
...
*/
  ...
  describe('the logout method', function() {
    ...
    beforeEach(function() {
      return User.login(this.data.username, this.data.password);
    });
