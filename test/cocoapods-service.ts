...
# End Podfile
			...
			{
				testCaseDescription:
					"replaces correctly special chars from plugin's name",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
...
# End Podfile
			...
			{
				testCaseDescription:
					"replaces correctly special chars from plugin's name when plugin has _",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
	target 'MyAppTests' do
...
# End Podfile
			...
			{
				testCaseDescription:
					"treats plugin1_plugin and plugin1___plugin as the same plugin (by design as we do not expect plugins to have names with three underscores)",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
end
...
# End Podfile
			...
			{
				testCaseDescription:
					"replaces the plugin's old Podfile with the new one inside project's Podfile",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
	target 'MyAppTests' do
...
# End Podfile
			...
			{
				testCaseDescription:
					"adds plugin with postinstall when project's Podfile has content, but does not have postinstall",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
end
...
# End Podfile
			...
			{
				testCaseDescription:
					"merges more than one hooks with block parameter correctly.",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
	target 'MyAppTests' do
...
# End Podfile
			...
			{
				testCaseDescription:
					"merges more than one hooks with and without block parameter correctly",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
	target 'MyAppTests' do
...
# End Podfile
			...
			{
				testCaseDescription:
					"should not change the Podfile when the plugin content is already part of the project",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
	target 'MyAppTests' do
...
# End Podfile
	...
	describe("removes plugin's Podfile correctly", () => {
		const testCases: IMergePodfileHooksTestCase[] = [
			{
				testCaseDescription:
					"removes plugin's Podfile from project's one and deletes project's Podfile as nothing is left there",
				input: `
pod 'GoogleAnalytics', '~> 3.1'
`,
				output: null,
...
# End Podfile
			...
			{
				testCaseDescription:
					"removes plugin's Podfile (with hook) from project's one and deletes project's Podfile as nothing is left there",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
	target 'MyAppTests' do
...
# End Podfile
			...
			{
				testCaseDescription:
					"removes Podfile which has several postinstall hooks and deletes project's Podfile as nothing is left there",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
	target 'MyAppTests' do
...
# End Podfile
			...
			{
				testCaseDescription:
					"removes plugin's Podfile (with hook) from project's one when there are other plugins with hooks in the project Podfile",
				input: `
target 'MyApp' do
	pod 'GoogleAnalytics', '~> 3.1'
	target 'MyAppTests' do
...
# End Podfile
	...
	describe("executePodInstall", () => {
		...
		["pod", "sandbox-pod"].forEach((podExecutable) => {
			it(`uses ${podExecutable} executable when USE_POD_SANDBOX is ${
				podExecutable === "sandbox-pod"
			}`, async () => {
		...
		it("calls pod install spawnFromEvent with correct arguments", async () => {
			...
			childProcess.spawnFromEvent = async (
				...
				assert.deepStrictEqual(spawnFromEventOptions, { throwError: false });
				return {
					stdout: "",
					stderr: "",
					exitCode: 0,
	...
	describe("remove duplicated platfoms from project podfile", () => {
		...
		function setupMocks(
			...
			projectData.getAppResourcesDirectoryPath = () =>
				"my/full/path/to/app/App_Resources";
			...
			fs.exists = (filePath: string) =>
				projectPodfilePath === filePath || _.includes(podsPaths, filePath);
		...
		_.each(testCasesWithApplyAndRemove, (testCase) => {
			it(testCase.name, async () => {
				...
				for (const pod of testCase.podsToRemove) {
					await cocoapodsService.removePodfileFromProject(
						pod.name,
						pod.path,
						projectData,
						projectPodfilePath
...
# End Podfile
	...
	describe("override pods", () => {
		it("should override pods", async () => {
			const projectDataMock = _.assign({}, mockProjectData, {
				nsConfig: {
					overridePods: true,
				},
			});
...
# End Podfile
				...
				expectedFinalOutput: `use_frameworks!
...
# End Podfile
				...
				expectedIntermidiatOutput: `use_frameworks!
...
# End Podfile
		...
		it("should not override if not in app resources podfile and the override when added", async () => {
			const testCase = {
				...
				updatedAppResourcesPodContent: `pod 'MaterialComponents/Tabs', '~> 84.4'
pod 'Mapbox-iOS-SDK', '~> 4.4.1'`,
				projectPodfileContent: "",
				expectedOutput: `use_frameworks!
