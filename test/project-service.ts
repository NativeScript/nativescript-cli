...
class ProjectIntegrationTest implements IProjectIntegrationTest {
	...
	public getDefaultTemplatePath(): IFuture<string> {
		return (() => {
			...
			var defaultTemplatePath = path.join(npm.getCacheRootPath().wait(), defaultTemplatePackageName);
			...
			if(!fs.exists(path.join(defaultTemplatePath, latestVersion)).wait()) {
				npm.addToCache(util.format("%s@%s", defaultTemplatePackageName, latestVersion)).wait();
			}
...
export class ProjectIntegrationTestFactory {
	public static create(): IProjectIntegrationTest {
		return new ProjectIntegrationTest();
	}
}

describe("Project Service Tests", () => {
	describe("project service integration tests", () => {
		var projectIntegrationTest: IProjectIntegrationTest;
		beforeEach(() => {
			projectIntegrationTest = ProjectIntegrationTestFactory.create();
		});
		afterEach(() => {
			projectIntegrationTest.dispose();
		});
