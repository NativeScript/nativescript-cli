...
export class CompanyInsightsController implements ICompanyInsightsController {
	...
	public async getCompanyData(): Promise<ICompanyData> {
		...
		try {
			...
			this.$logger.trace(
				`Unable to get current public ip address. Error is: `,
				err
			);
		}
	...
	@cache()
	private async getCompanyDataFromPlaygroundInsightsEndpoint(
		ipAddress: string
		...
		try {
			const url = util.format(
				this.$config.INSIGHTS_URL_ENDPOINT,
			...
			if (data.company) {
				const industries = _.isArray(data.company.industries)
					? data.company.industries.join(AnalyticsEventLabelDelimiter)
					: null;
				companyData = {
