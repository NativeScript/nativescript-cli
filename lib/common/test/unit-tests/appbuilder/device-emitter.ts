import { withDone, DoneCallback } from "../../with-done";
import { Yok } from "../../../yok";
...
describe("deviceEmitter", () => {
	...
	describe("raises correct events after initialize is called:", () => {
		...
		_.each(
			...
			(deviceEvent) => {
				describe(deviceEvent, () => {
					...
					it(
						"is raised when working with device",
						withDone((done) => {
		...
		describe("openDeviceLogStream", () => {
			...
			it(
				"is called when working with device",
				withDone((done) => {
		...
		describe("deviceLogProvider on data", () => {
			...
			describe(`raises ${DEVICE_LOG_EVENT_NAME} with correct identifier and data`, () => {
				...
				it(
					"is called when device reports data",
					withDone((done) => {
			...
			(applicationEvent: string) => {
				describe(applicationEvent, () => {
					...
					it(
						"is raised when working with device",
						withDone((done) => {
							const expectedDebuggableViewInfo: Mobile.IDebugWebViewInfo =
								createDebuggableWebView("test1");
