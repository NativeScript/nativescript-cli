...
export class PreviewAppFilesService implements IPreviewAppFilesService {
	...
	public getInitialFilesPayload(
		...
		const payloads = this.getFilesPayload(
			data,
			{ filesToSync },
			platform,
			deviceId
	...
	public getFilesPayload(
		...
		const filesToTransfer = filesToSync
			...
			.filter(
				(file) => !_.includes(this.excludedFileExtensions, path.extname(file))
			);

		this.$logger.trace(`Sending ${filesToTransfer.join("\n")}.`);

		const files = this.createFilePayloads(
			data,
			platform,
			filesToTransfer,
			filesToRemove
	...
	private createFilePayloads(
		...
		const payloadsToSync = _.filter(filesToTransfer, (file) => {
			const fileInfo = this.$projectFilesProvider.getProjectFileInfo(
				file,
				platform,
				{}
			);
			...
			this.createFilePayload(
				file,
				platform,
				rootFilesDir,
				PreviewSdkEventNames.CHANGE_EVENT_NAME
			)
		);
		const payloadsToRemove = _.map(filesToRemove, (file) =>
			this.createFilePayload(
				file,
				platform,
				rootFilesDir,
				PreviewSdkEventNames.UNLINK_EVENT_NAME
			)
		);
