...
export class PreviewQrCodeService implements IPreviewQrCodeService {
	...
	public async getPlaygroundAppQrCode(
		...
		if (
			...
			result.android = await this.getLiveSyncQrCode(
				this.getGooglePlayUrl(projectDir)
			);
		}
		...
		if (
			...
			result.ios = await this.getLiveSyncQrCode(
				this.getAppStoreUrl(projectDir)
			);
		}
	...
	public async printLiveSyncQrCode(
		options: IPrintLiveSyncOptions
	): Promise<void> {
...
To scan the QR code and deploy your app on a device, you need to have the \`NativeScript Playground app\`:
	...
	private async getShortenUrl(url: string): Promise<string> {
		const shortenUrlEndpoint = util.format(
			this.$config.SHORTEN_URL_ENDPOINT,
