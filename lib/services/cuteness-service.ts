...
export class CutenessService implements ICutenessService {
	...
	public get cutenessAppPath(): IFuture<string> {
		return this.getCutenessAppPath();
	}

	private getCutenessAppPath(): IFuture<string> {
		return (() => {
			try {
			...
			var latestMasterSha = masterRef.object.sha.substr(0, 7);
			...
			var cutenessAppPath = path.join(downloadDir, util.format("telerik-%s-%s", CutenessService.GITHUB_CUTENESS_REPO_NAME, latestMasterSha));

			if(this.$fs.exists(cutenessAppPath).wait()) {
				this.$logger.trace("Cuteness app already exists. No need to download.");
			} else {
				this.$logger.trace("Downloading %s cuteness app", latestMasterSha);
